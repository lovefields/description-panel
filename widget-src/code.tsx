...
const { widget } = figma;
const { useSyncedState, usePropertyMenu, AutoLayout, Text, Rectangle, Input, useEffect } = widget;

function Widget() {
    const [widgetData, setWidgetData] = useSyncedState("widgetData", "");
    const [pageTitle, setPageTitle] = useSyncedState("pageTitle", "");
    const [pageCaption, setPageCaption] = useSyncedState("pageCaption", "");
    let widgetDataJson = {
        visible: [{ id: 0, type: 1, content: "test" }],
        invisible: [],
        tracking: [],
    };
    ...
    usePropertyMenu(
        ...
        () => {
            return new Promise((resolve) => {
                ...
                figma.ui.postMessage(JSON.stringify({ mode: "new" }));
            });
        }
    );

    return (
        <AutoLayout name="wrap" direction={"vertical"} spacing={8} padding={16} cornerRadius={8} fill={"#FFFFFF"} stroke={"#E6E6E6"}>
            <AutoLayout name="Page information" direction={"vertical"}>
                <Input
                    value={pageTitle}
            ...
            <AutoLayout name="id" width={20} height={20} horizontalAlignItems={"center"} verticalAlignItems={"center"} padding={2} cornerRadius={50}>
                <Text fontSize={16} horizontalAlignText={"center"}></Text>
            </AutoLayout>
        </AutoLayout>
    );
}
